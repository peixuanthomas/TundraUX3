...
pub struct RecoveryAction {
    ...
    pub outcome_path: PathBuf,
    /// A one-time random value generated by the launcher and echoed only by
    /// WezTerm after its native recovery scene observes Enter. This prevents a
    /// plain window exit from being mistaken for an explicit restart.
    pub restart_credential: String,
}
    ...
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::Recovery {
                incident_id,
                message,
            } => write!(formatter, "recovery incident {incident_id}: {message}"),
        }
    }
}
    ...
    R: ResetCallback,
{
    ...
    fn recover(
        ...
        let restart_credential =
            recovery_restart_credential().map_err(|error| LauncherError::Recovery {
                incident_id: recovery_incident_id.clone(),
                message: error,
            })?;
        // Native recovery creates this file with create_new semantics after
        // Enter. Remove an abandoned file before launch; a racing replacement
        // can only make recovery fail closed.
        ...
        let mut recovery =
            self.factory
                ...
                .map_err(|error| LauncherError::Recovery {
                    incident_id: action.incident_id.clone(),
                    message: error.to_string(),
                })?;
        let status = match recovery
            .wait_or_shutdown(self.shutdown_flag())
            .map_err(|error| LauncherError::Recovery {
                incident_id: action.incident_id.clone(),
                message: error.to_string(),
            })? {
        ...
        let _ = fs::remove_file(handoff_path);
        let restart_requested = read_recovery_restart(
            &recovery_outcome_path,
            &action.incident_id,
            &action.restart_credential,
        );
        let _ = fs::remove_file(recovery_outcome_path);
        match (status.code, restart_requested) {
            // Native recovery may finish its GUI event loop with zero after
            // recording Enter, or propagate the private restart status 74.
            (Some(NORMAL_EXIT | RESTART_EXIT), true) if status.signal.is_none() => {
                Ok(RecoveryDecision::Restart)
            }
            _ => Err(LauncherError::Recovery {
                incident_id: action.incident_id,
                message: "screen closed without an Enter restart request".to_owned(),
            }),
        }
    }
...
fn read_recovery_restart(path: &Path, incident_id: &str, restart_credential: &str) -> bool {
    ...
    let Ok(_) = Read::by_ref(&mut file)
        .take(MAX_RECOVERY_OUTCOME_BYTES + 1)
    ...
    value.schema_version == 1
        ...
        && value.origin == "wezterm-native-recovery"
        && value.kind == "restart"
        && value.code == RESTART_EXIT
        && value.restart_credential == restart_credential
}

/// A recovery credential is scoped to one launcher-created incident. It is
/// passed only in the private bundled GUI's inherited environment and is never
/// included in argv, incidents, handoffs, diagnostics, or the panic capsule.
fn recovery_restart_credential() -> Result<String, String> {
    let mut bytes = [0_u8; 32];
    rand::rngs::OsRng
        .try_fill_bytes(&mut bytes)
        .map_err(|error| format!("secure recovery random source failed: {error}"))?;
    let mut credential = String::with_capacity(bytes.len() * 2);
    for byte in bytes {
        use std::fmt::Write as _;
        let _ = write!(credential, "{byte:02x}");
    }
    Ok(credential)
}
...
impl BundledWezTerm {
    ...
    fn base_command(&self) -> ProcessSpec {
    ...
    fn session_command(&self, foreground: &Path) -> ProcessSpec {
    ...
    fn command_for_session(&self, session: &SessionSpec) -> ProcessSpec {
        self.session_command(&self.layout.shell)
    ...
    fn command_for_recovery(&self, action: &RecoveryAction) -> ProcessSpec {
        // This private WezTerm command is intentionally not `start -- …`:
        // native recovery owns the scene and must not allocate a terminal PTY
        // or spawn the retired tundra-recovery helper.
        self.base_command()
            .args([
                "tundra-recovery".to_owned(),
                "--handoff".to_owned(),
                action.handoff_path.to_string_lossy().into_owned(),
                "--outcome".to_owned(),
                action.outcome_path.to_string_lossy().into_owned(),
                "--incident-id".to_owned(),
                action.incident_id.clone(),
            ])
            .env(
                "TUNDRA_RECOVERY_RESTART_CREDENTIAL",
                &action.restart_credential,
            )
    }
}
...
pub fn show_critical_fallback(error: &LauncherError) {
    let (incident_id, source) = match error {
        LauncherError::Recovery { incident_id, .. } if valid_incident_id(incident_id) => {
            (incident_id.clone(), "tundra-native-recovery")
        }
        _ => (
            format!("startup-{}-{}", unix_seconds(), std::process::id()),
            "tundra-launcher-startup",
        ),
    };
    ...
    let _ = native_platform().show_critical_error(
        "TundraUX3 could not start or recover",
        &format!(
...
mod tests {
    ...
    impl ChildFactory for FakeFactory {
        ...
        fn launch_recovery(
            ...
            fs::write(
                &action.outcome_path,
                format!(
                    "{{\"schema_version\":1,\"incident_id\":\"{}\",\"origin\":\"wezterm-native-recovery\",\"kind\":\"restart\",\"code\":74,\"restart_credential\":\"{}\"}}",
                    action.incident_id, action.restart_credential
                ),
            )
            .unwrap();
    ...
    fn recovery_gui_exit_without_enter_credential_is_not_a_restart() {
        ...
        assert!(!read_recovery_restart(&path, "incident-1", "credential-1"));
        fs::write(
            &path,
            "{\"schema_version\":1,\"incident_id\":\"other\",\"origin\":\"wezterm-native-recovery\",\"kind\":\"restart\",\"code\":74,\"restart_credential\":\"credential-1\"}",
        )
        .unwrap();
        assert!(!read_recovery_restart(&path, "incident-1", "credential-1"));
        fs::write(
            &path,
            "{\"schema_version\":1,\"incident_id\":\"incident-1\",\"origin\":\"wezterm-native-recovery\",\"kind\":\"restart\",\"code\":74,\"restart_credential\":\"wrong\"}",
        )
        .unwrap();
        assert!(!read_recovery_restart(&path, "incident-1", "credential-1"));
        fs::write(
            &path,
            "{\"schema_version\":1,\"incident_id\":\"incident-1\",\"origin\":\"wezterm-native-recovery\",\"kind\":\"restart\",\"code\":74,\"restart_credential\":\"credential-1\",\"unexpected\":true}",
        )
        .unwrap();
        assert!(!read_recovery_restart(&path, "incident-1", "credential-1"));
        fs::write(
            &path,
            "{\"schema_version\":1,\"incident_id\":\"incident-1\",\"origin\":\"wezterm-native-recovery\",\"kind\":\"restart\",\"code\":74,\"restart_credential\":\"credential-1\"}",
        )
        .unwrap();
        assert!(read_recovery_restart(&path, "incident-1", "credential-1"));
        let _ = fs::remove_dir_all(root);
    ...
    fn recovery_command_binds_the_expected_incident_out_of_band() {
        ...
        let action = RecoveryAction {
            ...
            restart_credential: "credential-42".to_owned(),
        };
        ...
        assert!(
            !command
                .args_slice()
                .iter()
                .any(|argument| argument == "start" || argument.contains("tundra-recovery.exe"))
        );
        assert!(
            command
                .env_map()
                .get("TUNDRA_RECOVERY_HANDOFF_PATH")
                .is_none()
        );
        assert_eq!(
            command
                .env_map()
                .get("TUNDRA_RECOVERY_RESTART_CREDENTIAL")
                .map(String::as_str),
            Some("credential-42")
        );
        assert!(
            !command.args_slice().iter().any(|argument| {
                argument == "--restart-credential" || argument == "credential-42"
            })
        );
        let _ = fs::remove_dir_all(root);
    }
    ...
    #[test]
    fn recovery_credentials_are_random_lower_hex() {
        let first = recovery_restart_credential().unwrap();
        let second = recovery_restart_credential().unwrap();
    ...
    #[test]
    fn only_safe_incident_ids_can_be_used_as_fallback_log_names() {
        assert!(valid_incident_id("panic-session_42"));
        assert!(!valid_incident_id("../panic-session"));
        assert!(!valid_incident_id("panic/session"));
        assert!(!valid_incident_id(&"x".repeat(129)));
    }
