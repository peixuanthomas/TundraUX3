...
fn release_policy_rejects_an_explicit_debug_home() {
    ...
    startup.debug_policy = identity::DebugPolicy { debug_build: false };
...
fn release_policy_rejects_a_restored_debug_home() {
    ...
    let state = ShellSession::new_with_startup(build_default_config(), (120, 40), startup);
...
fn modal_shortcuts_reject_ctrl_alt_and_super_modifiers() {
    ...
    let id = state.notify_modal(
        ...
        vec![
            ShellNotificationAction::new("confirm", "Confirm").with_shortcut(InputKey::Char('y')),
    ...
    for modifiers in modified_shortcuts {
        state.apply_input(key_input(InputKey::Char('y'), modifiers, InputPhase::Press));
    ...
    assert_eq!(
        ...
        Some(shell::ShellNotificationResponse {
            notification_id: id,
...
fn modal_shortcut_shift_is_only_accepted_for_ascii_letter_case() {
    ...
    state.notify_modal(
        ...
        vec![ShellNotificationAction::new("space", "Continue").with_shortcut(InputKey::Char(' '))],
    );
...
fn modal_activation_rejects_repeat_and_release_but_repeat_navigation_works() {
    ...
    for (key, phase) in [
        ...
        (InputKey::Char('y'), InputPhase::Repeat),
        (InputKey::Char('y'), InputPhase::Release),
    ] {
...
fn undersized_notification_rejects_keyboard_and_mouse_activation() {
    for shrink_width in [true, false] {
        ...
        assert!(matches!(
            ui::notification_layout(Rect::new(0, 0, too_small_size.0, too_small_size.1), &model,),
...
fn time_sync_modal_close_and_programmatic_dismiss_restore_previous_focus() {
    let mut closed = ShellSession::new(debug_config(), (120, 40));
    ...
    let mut dismissed = ShellSession::new(debug_config(), (120, 40));
...
fn current_time_label_uses_configured_timezone_datetime() {
    ...
    let mut state = ShellSession::new_with_startup(build_default_config(), (120, 40), startup);
    let utc = Utc
...
fn debug_override_wins_over_persisted_config_and_session() {
    let startup = ShellStartupState {
        ...
        login_users: Vec::new(),
        debug_policy: identity::DebugPolicy::default(),
    };
...
fn restored_session_is_sanitized_to_stable_home_state() {
    let startup = ShellStartupState {
        ...
        restored_session: Some(ShellRestoredSession {
            ...
            active_popup: Some(shell::ShellPopup {
...
fn home_entry_coordinates(state: &ShellSession, index: usize) -> (u16, u16) {
...
fn component_coordinates(state: &ShellSession, component: ShellComponent) -> (u16, u16) {
    let region = state
        .hit_map()
        .regions()
        .iter()
...
fn key_input(key: InputKey, modifiers: InputModifiers, phase: InputPhase) -> InputEvent {
    InputEvent::Key(KeyInput::with_phase(key, modifiers, phase))
}

fn selected_notification_action(state: &ShellSession) -> Option<usize> {
    state
        .to_notification_view_model()?
        .actions
        .iter()
...
fn state_with_two_action_modal() -> (ShellSession, u64) {
    let mut state = ShellSession::new(debug_config(), (120, 40));
    let id = state.notify_modal(
        "Confirm",
        "Continue?",
...
fn notification_dialog_layout(state: &ShellSession, area: Rect) -> ui::NotificationDialogLayout {
    let model = state
        .to_notification_view_model()
        .expect("active notification view model");
    match ui::notification_layout(area, &model) {
...
fn notification_action_coordinates(state: &ShellSession, index: usize) -> (u16, u16) {
...
fn notification_action_coordinates_in(
    state: &ShellSession,
    area: Rect,
    index: usize,
) -> (u16, u16) {
...
fn focus_clock_button(state: &mut ShellSession) {
    state.apply_input(InputEvent::from_key_label("Tab"));
    assert_eq!(state.focused_component(), ShellComponent::ClockButton);
}
