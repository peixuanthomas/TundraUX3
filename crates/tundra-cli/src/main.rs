...
fn main() {
    ...
    let _ = process_watchdog
        .register_emergency_cleanup(Arc::new(tundra_weathr::restore_terminal_best_effort));
    ...
    let weathr_watchdog =
        match process_watchdog.register_app(tundra_weathr::weathr_watchdog_descriptor()) {
            Ok(watchdog) => watchdog,
            Err(error) => {
