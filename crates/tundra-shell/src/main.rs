use std::sync::Arc;
use tundra_watchdog::{ProcessWatchdog, WatchdogConfig, WatchdogRuntime};
...
fn main() {
    ...
    let _ = process_watchdog
        .register_emergency_cleanup(Arc::new(tundra_shell::restore_terminal_best_effort));

    let mut stdout = std::io::stdout();
    let exit_code = match tundra_shell::parse_shell_args(std::env::args().skip(1)) {
        Ok(config) => {
            match tundra_shell::run_shell_blocking_managed(&mut stdout, config, process_watchdog) {
                Ok(()) => 0,
                Err(error) => {
...
fn start_watchdog() -> Result<(WatchdogRuntime, ProcessWatchdog), tundra_watchdog::WatchdogError> {
