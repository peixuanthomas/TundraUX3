#[derive(Debug)]
enum LauncherRefreshEvent {
    ItemChecked {
        request_id: u64,
        id: String,
        result: Result<DomainLauncherItemStatus, String>,
    },
    Finished {
        request_id: u64,
        error: Option<String>,
...
struct ShellLauncherTaskShared {
    platform: std::sync::Arc<dyn Platform>,
    task_group: tundra_watchdog::ManagedTaskGroup,
    event_tx: std::sync::mpsc::Sender<LauncherRefreshEvent>,
    event_rx: std::sync::Mutex<std::sync::mpsc::Receiver<LauncherRefreshEvent>>,
    workers: std::sync::Mutex<
        std::collections::BTreeMap<u64, tundra_watchdog::ManagedThreadHandle<()>>,
    >,
...
#[derive(Clone)]
struct ShellLauncherTaskRuntime {
    shared: std::sync::Arc<ShellLauncherTaskShared>,
}
...
static NEXT_LAUNCHER_RUNTIME_ID: std::sync::atomic::AtomicU64 =
    std::sync::atomic::AtomicU64::new(1);
const MAX_CONCURRENT_LAUNCHER_REFRESHES: usize = 2;

impl ShellLauncherTaskRuntime {
    fn new_managed(
        platform: std::sync::Arc<dyn Platform>,
        watchdog: tundra_watchdog::AppWatchdog,
    ) -> Self {
    ...
    fn submit(
        &self,
        entries: Vec<tundra_storage::LauncherEntryRecord>,
    ) -> Result<u64, String> {
        use std::sync::atomic::Ordering;
        ...
        let request_id = self
            ...
            .fetch_add(1, Ordering::Relaxed)
            .max(1);
        let task_id = tundra_watchdog::TaskId::new(format!(
            "refresh-{}",
            request_id % 64
        ))
        ...
        let worker = self
            .shared
            .task_group
            .spawn_thread(tundra_watchdog::TaskSpec::one_shot(task_id), move || {
                let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
                    for entry in &entries {
                        let result = tundra_apps::launcher::verify_launcher_entry(
                            entry,
                            platform.as_ref(),
                        )
    ...
    fn drain_events(&self) -> Vec<LauncherRefreshEvent> {
...
impl ShellState {
    fn poll_launcher_background_tasks(&mut self) {
        let events = self
        ...
        for event in events {
            match event {
                LauncherRefreshEvent::ItemChecked {
                    ...
                    let Some(state) = self.launcher_state.as_mut() else {
                        continue;
                    };
                    match result {
                        Ok(status) => state.set_item_status(&id, status),
                        Err(error) => {
                            state.set_item_status(&id, DomainLauncherItemStatus::Unsupported);
...
mod launcher_task_workflow_tests {
    ...
    fn managed_refresh_publishes_full_integrity_results() {
        ...
        let platform = tundra_platform::mock::MockPlatform::new(user_dirs, app_paths)
            .with_kind(tundra_platform::PlatformKind::Windows);
        platform.set_file_attributes(
            executable.clone(),
            tundra_platform::FileAttributes {
        ...
        platform.set_file_open_policy(
            executable.clone(),
            tundra_platform::FileOpenPolicy::launcher_required(
                tundra_platform::ExecutableKind::NativeBinary,
                "test policy",
            ),
        );
        let fingerprint = tundra_apps::launcher::fingerprint_file(&executable)
            .expect("approved fingerprint");
        let entry = tundra_storage::LauncherEntryRecord {
            id: "program".to_string(),
            path: executable.to_string_lossy().into_owned(),
            executable_kind: Some(tundra_storage::LauncherExecutableKind::NativeBinary),
            fingerprint: Some(fingerprint),
            added_by_user_id: "admin".to_string(),
            added_at_epoch_ms: 0,
        };
        let watchdog = default_editor_watchdog().expect("test watchdog");
        let runtime = ShellLauncherTaskRuntime::new_managed(
        ...
        assert_eq!(status, Some(DomainLauncherItemStatus::Ready));
        drop(runtime);
        tundra_platform::cleanup_temp_path(&root).expect("cleanup test fixture");
    }
}
