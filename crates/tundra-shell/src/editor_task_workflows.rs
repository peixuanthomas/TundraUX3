...
enum EditorTaskAccess {
    Editable,
    ReadOnly,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum EditorTaskStage {
    Inspecting,
    Reading,
    Decoding,
    ParsingMarkdown,
...
#[derive(Debug)]
struct EditorLoadedTaskDocument {
    state: EditorState,
    fingerprint: DocumentFingerprint,
    total_bytes: u64,
    rich_blocks: Option<std::sync::Arc<[tundra_ui::EditorRenderBlock]>>,
}

#[derive(Debug)]
struct EditorSaveTaskRequest {
    id: u64,
    path: PathBuf,
...
#[derive(Debug)]
enum EditorSaveTaskError {
    ExternalModification,
    Write(String),
}

#[derive(Debug)]
enum EditorTaskEvent {
    Progress {
        id: u64,
...
struct ShellEditorTaskShared {
    task_group: Option<ManagedTaskGroup>,
    event_tx: std_mpsc::Sender<EditorTaskEvent>,
    event_rx: StdMutex<std_mpsc::Receiver<EditorTaskEvent>>,
    cancelled: StdArc<StdMutex<BTreeSet<u64>>>,
    workers: StdMutex<BTreeMap<u64, ManagedThreadHandle<()>>>,
    ...
    active_loads: StdArc<AtomicUsize>,
    active_load_bytes: StdArc<AtomicU64>,
}

const MAX_CONCURRENT_EDITOR_LOADS: usize = 2;
const EDITOR_WATCHDOG_LOAD_SLOTS: u64 = 64;
const MAX_IN_FLIGHT_EDITOR_LOAD_BYTES: u64 = tundra_platform::MAX_DOCUMENT_BYTES;
static NEXT_EDITOR_RUNTIME_ID: AtomicU64 = AtomicU64::new(1);
...
/// `ShellState`, whose equality is used by deterministic input tests.
#[derive(Clone)]
struct ShellEditorTaskRuntime {
    shared: StdArc<ShellEditorTaskShared>,
}
...
impl ShellEditorTaskRuntime {
    ...
    fn new_managed(watchdog: AppWatchdog) -> Self {
        let (event_tx, event_rx) = std_mpsc::channel();
    ...
    fn submit_load(&self, id: u64, path: PathBuf, access: EditorTaskAccess) -> Result<(), String> {
        let task_group = self
            .shared
            .task_group
            .clone()
    ...
    fn submit_save(
        ...
        let worker = task_group
            .spawn_thread(TaskSpec::one_shot(task_id), move || {
                ...
                let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
                    tundra_platform::atomic_write_document_if_unchanged_with(
                        &request.path,
                        request.expected,
                        |writer| request.snapshot.write_to(writer),
                    )
                    .map_err(|error| match error {
                        tundra_platform::DocumentWriteError::ExternalModification { .. } => {
    ...
    fn drain_events(&self) -> Vec<EditorTaskEvent> {
...
fn acquire_editor_load_slot(slots: &AtomicUsize) -> Option<usize> {
    loop {
...
fn default_editor_watchdog() -> Option<AppWatchdog> {
    ...
    FALLBACK
        .get_or_init(|| {
            ...
            let (runtime, process) = tundra_watchdog::WatchdogRuntime::start(config).ok()?;
...
fn load_editor_document_task(
    ...
    if total_bytes > tundra_platform::MAX_DOCUMENT_BYTES {
        return Err(format!(
    ...
    let loaded = {
        ...
        if is_log_document_path(&request.path) {
            tundra_platform::read_document_prefix_snapshot_limited_with_progress(
                &request.path,
                tundra_platform::MAX_DOCUMENT_BYTES,
                &mut progress,
            )
        } else {
            tundra_platform::read_document_bytes_limited_with_progress(
                &request.path,
                tundra_platform::MAX_DOCUMENT_BYTES,
                &mut progress,
            )
    ...
    let loaded = loaded.map_err(|error| {
        if let Some(error) = budget_error.take() {
            error
        } else if matches!(error, tundra_platform::PlatformError::Interrupted { .. })
            && is_cancelled()
        {
    ...
    if tundra_apps::editor::DocumentKind::from_path(&request.path)
...
static NEXT_EDITOR_TASK_ID: AtomicU64 = AtomicU64::new(1);

fn next_editor_task_id() -> u64 {
