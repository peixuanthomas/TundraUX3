...
impl RichPosition {
    ...
    /// Constructs a position from a persisted/raw node identifier.
    pub const fn from_raw(container_id: u64, grapheme_offset: usize) -> Self {
        Self::new(NodeId::new(container_id), grapheme_offset)
    }

    /// Constructs a position in an already-typed rich document node.
    pub const fn in_node(container_id: NodeId, grapheme_offset: usize) -> Self {
        Self::new(container_id, grapheme_offset)
    }
}
...
impl RichRange {
    ...
    /// Constructs a range within one rich document node from grapheme offsets.
    pub const fn in_node(container_id: NodeId, start: usize, end: usize) -> Self {
        Self::new(
            RichPosition::in_node(container_id, start),
            RichPosition::in_node(container_id, end),
        )
    }

    /// Constructs a range in a node identified by a persisted/raw identifier.
    pub const fn from_raw(container_id: u64, start: usize, end: usize) -> Self {
        Self::in_node(NodeId::new(container_id), start, end)
    }

    /// Alias for constructing a range whose endpoints may be in different nodes.
    ...
    /// Compatibility name for an empty/collapsed selection.
...
#[cfg(test)]
#[path = "../../tests/unit/editor/rich_document/tests.rs"]
mod tests;
