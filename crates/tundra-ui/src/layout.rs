...
use crate::view_model::{NotificationActionViewModel, NotificationViewModel};
...
pub fn notification_layout(area: Rect, model: &NotificationViewModel) -> NotificationLayout {
    let inner_width = NOTIFICATION_DIALOG_WIDTH.saturating_sub(NOTIFICATION_DIALOG_BORDER_CELLS);
    ...
    if area.width < NOTIFICATION_DIALOG_WIDTH || area.height < required_height {
        return NotificationLayout::TooSmall {
            required_width: NOTIFICATION_DIALOG_WIDTH,
            required_height,
        };
    }

    let dialog = centered_rect(area, NOTIFICATION_DIALOG_WIDTH, required_height);
    let inner = Rect::new(
