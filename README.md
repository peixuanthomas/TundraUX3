...
Settings 的 Editor 分类可以配置 Explorer 交给内置编辑器打开的文件后缀。后缀匹配不区分大小写，支持 `.d.ts` 这类复合后缀；清空列表后，Explorer 会把所有文件交给系统默认应用。

### 时钟、设置和诊断

共享 time crate 以最近一次同步 UTC 和单调时钟为锚点生成快照，再通过 chrono-tz 投影到配置时区，因此 DST 跳变不会被硬编码成固定偏移。时钟的用户项目写入 clock.v1.json。Settings 的“时间设置”可以选择通过平台抽象层读取操作系统时间，或使用默认/自定义 HTTP(S) 时间服务器；自定义地址只有在返回有效 `Date` 响应头、确认能够同步后才会保存，否则会显示错误弹窗。

Settings 的全局选项写入 StorageConfig 并同步更新 AppState；外观选项写入当前用户账户，并立即更新活动主题。Diagnostics 汇总平台能力、存储文档健康、watchdog 报告和日志；修复操作先生成预览，再由用户确认执行。
...
`tundra-shell` 不接受任何命令行参数（包括 `--help`），传入任意参数都会使进程以参数错误退出。Shell 固定进入全屏 UI，应用选择和 Editor 文件打开只能从 UI 发起。`tundra-cli` 是独立的管理工具，可以读写公开配置，但不能向 Shell 传参或绕过 UI 启动 Editor。
